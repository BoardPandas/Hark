...
- [x] **Windows dev box smoke test (2026-07-16):** default input is 48 kHz F32 (as predicted), the LL hook installs, and the Deepgram pre-warm measured 218 ms on this network.
- [x] **CP6 interactive gate PASSED (user-validated 2026-07-16):** hold LCtrl+LWin / speak / release injects the transcript at the cursor with no issues reported: pre-roll, clipboard restore, and the injected-paste filter all behaved. Phase 1 definition of done (Windows) is met; CP7 (macOS) deferred until Mac hardware is available.
- [x] **Doppler env-name mismatch for dev runs:** the Doppler project stores provider-named secrets (`DEEPGRAM_API_KEY`), but Hark reads `HARK_STT_KEY`, so a bare `doppler run` does not wire the key. Map it explicitly: `$env:HARK_STT_KEY = (doppler secrets get DEEPGRAM_API_KEY --project hark --config prd --plain)`.
