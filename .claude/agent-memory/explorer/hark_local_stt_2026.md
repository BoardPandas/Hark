...
**Important simplification vs the 2026-07-15 research:** the open sherpa-onnx reliability bug (issue #3267, `modified_beam_search` hallucinating ~20% of the time) blocked *decode-time hotwords*. That blocker is now moot for the local-STT feature — Hark's spellbook correction already runs as a provider-agnostic **phonetic post-correction pass** (`rphonetic` + `strsim`, per the cloud-STT spellbook design) that applies equally to local-model output. So the local engine only ever needs to run `greedy_search`, which is the reliable path; there's no reason to touch `modified_beam_search`/hotwords for local STT at all. This removes the single biggest open risk from the prior sherpa-onnx research.
