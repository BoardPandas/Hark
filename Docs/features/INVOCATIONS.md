...
> **Related Pages**: [Spellbook](SPELLBOOK.md), [Voice Cleanup](VOICE_CLEANUP.md), [Desktop UI](DESKTOP_UI.md), [Configuration and Secrets](../core/CONFIGURATION.md)
...
The pass runs on the pipeline worker thread between spellbook pass 1 and the optional cleanup call ([worker.rs:222-230](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-pipeline/src/worker.rs#L222-L230)). Running after pass 1 is deliberate: pass 1 canonicalizes the *spoken* words, which can only help a trigger match, and the expansion is inserted afterwards so the spellbook never rewrites it ([worker.rs:349-356](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-pipeline/src/worker.rs#L349-L356)).

```mermaid
graph TD
    A["STT transcript"] --> B["Spellbook pass 1"]
    B --> C["Invocation pass"]
    C --> D{"Trigger fired?"}
    D -->|"Yes"| E["Canned text verbatim"]
    D -->|"No"| F["Cleanup voice"]
    F --> G["Spellbook pass 2"]
...
- **Verbatim injection.** The expansion is moved into the result string untouched; nothing downstream rewrites it ([expander.rs:159-165](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L159-L165)).
...
Sources: [worker.rs:218-270](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-pipeline/src/worker.rs#L218-L270), [expander.rs:1-30](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L1-L30), [lib.rs:340-360](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-store/src/lib.rs#L340-L360)
...
`Expander` lives in the spellbook crate and is a second consumer of the same guarded phonetic matcher that powers term correction, not a parallel implementation of it ([expander.rs:1-12](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L1-L12)). Reuse is a safety decision: Double Metaphone code equality alone collides common words with proper nouns, so the >= 4-char / all-alphabetic gate, the digit/short-word exact-only path, and the Jaro-Winkler confirmation all have to stay on the path.

The one thing the two consumers do differently is the confirmation threshold. `window_matches` takes it as a parameter, and each caller supplies its own ([matcher.rs:106-117](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/matcher.rs#L106-L117)):
...
|---|---|---|
| `Utterance` (default) | The trigger accounts for every token in the dictation | The expansion replaces the entire text ([expander.rs:155-165](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L155-L165)) |
| `Anywhere` | The trigger appears as a token window inside a longer dictation | The expansion is spliced over the matched byte span ([expander.rs:167-215](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L167-L215)) |

`expand` tokenizes once, then runs the utterance pass before the anywhere pass. A whole-dictation hit returns immediately, because the trigger has consumed every spoken word and nothing else can apply ([expander.rs:151-165](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L151-L165)). The anywhere pass reuses `Corrector::correct`'s consume-and-splice algorithm over byte spans, so punctuation and casing surrounding a match survive without a reattachment step ([expander.rs:167-172](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L167-L172)).

Entries are sorted longest-first so a multi-word trigger wins an overlap against a shorter one, matching the spellbook's ordering contract; the sort is stable, so equal-length triggers keep their configured order ([expander.rs:108-111](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L108-L111)).

Because matching runs *after* spellbook pass 1, a trigger word the provider keeps mishearing can be repaired for free by adding it to the Spellbook — the transcript is corrected before the trigger is ever compared ([expander.rs:20-25](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L20-L25)).

Sources: [expander.rs:14-215](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/expander.rs#L14-L215), [matcher.rs:14-135](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-spellbook/src/matcher.rs#L14-L135)
...
The regression test gives its `MockCleaner` an **empty script**, so any call to it panics — not panicking is the assertion. Its corrector is hostile in the same test, which simultaneously pins that spellbook pass 2 never runs over the expansion ([worker.rs:754-790](https://github.com/BoardPandas/Hark/blob/bcfcc3fef6f02252870fc3f06440d99992818ade/crates/hark-pipeline/src/worker.rs#L754-L790)).
